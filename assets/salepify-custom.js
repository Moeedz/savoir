const newRender = async () => {        
        ...
        for (let i = 0; i < unrenderedRules.length; i++) {
            ...
            if(claimedCount < canClaimCount && rule.auto_add_gift) {
                ...
                if(isCheapestOption){
                    ...
                    if(numberParentCanBeConverted >= rule.gift_qty * canClaimCount){
                        listRuleNeedToAddGift.push(ruleId);
                    }
                }else {
                    listRuleNeedToAddGift.push(ruleId);
                }
            }else if(claimedCount > canClaimCount && rule.auto_remove_gift){
            ...
            if(rule.show_free_gift_widget && window.tlAdvancedFreeGift.checkShowOnPage(rule.show_on_pages)){
                ...
                if (isGiftsOnlyLayout) {
                    ...
                    const headerTranslation = window.tlAdvancedFreeGift.translation.header;
                    ...
                    // Set innerHTML of add to cart
                    const addToCartTranslation = window.tlAdvancedFreeGift.translation.add_to_cart;
                    ...
                    bodyHtml += `
                        <div id="salepify-fg-advanced-main-section-${ruleId}"  
                            class="salepify-fg-advanced--detailed_list salepify-fg-advanced-main-section block--flex 
                                ...
                                ${conditionState === window.tlAdvancedFreeGift.CONDITION_STATE.MET_AFTER_CLAIM ? "salepify-fg-advanced-main-section--claimed" : ""}">
                            ...
                            <div class="salepify-fg-advanced-customer-buy block--flex block--collapsible">
                                ${window.tlAdvancedFreeGift.renderCustomerBuy(ruleId, listParentDisplayIds, parentMapRequiredQty)}
                            </div>
                            ...
                            <div class="salepify-fg-advanced-footer block--flex-inline flex--space-between block--collapsible"
                                ...
                                <div class="salepify-fg-advanced-footer__remain-gift-title">
                                    ...
                                    ${conditionState !== window.tlAdvancedFreeGift.CONDITION_STATE.MET_AFTER_CLAIM 
                                        ? window.tlAdvancedFreeGift.getTotalSelectedGift(ruleId) 
                                        : totalGift
                                    }/${totalGift}
                                </div>
                ...
                if(conditionState == window.tlAdvancedFreeGift.CONDITION_STATE.MET_BEFORE_CLAIM) availableToClaim.push(ruleId);
            }else {
        ...
        if(bodyHtml) {
            ...
            if (!isGiftsOnlyLayout) {
                popupIcon.style.display = window.tlAdvancedFreeGift.displaySettings.icon_popup.is_icon_displayed ? "block" : "none";
                window.tlAdvancedFreeGift.iconPopupElement.style.display = "flex";
                window.tlAdvancedFreeGift.iconPopupElement.style.backgroundImage = window.tlAdvancedFreeGift.displaySettings.icon_popup.is_icon_displayed
                    ? "none"
                    : `url(${window.tlAdvancedFreeGift.displaySettings.icon_popup.adv_free_gift_image.url})`;
            }
